...
export default function ScreenSize() {
  ...
  const [isLaptop, setIsLaptop] = useState<boolean>(
    window.innerWidth > mobileSize && window.innerWidth <= laptopSize
  );
  const [isBigLaptop, setIsBigLaptop] = useState<boolean>(
    window.innerWidth > mobileSize && window.innerWidth <= bigLaptopSize
  );
  const [isDesktop, setIsDesktop] = useState<boolean>(
    window.innerWidth > bigLaptopSize
  );
  ...
  return { isMobile, isTablet, isLaptop, isBigLaptop, isDesktop };
}
